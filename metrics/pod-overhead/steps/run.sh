...
get_overhead() {
	...
	while [ "$cid" == "null" ]; do
		if ((try >= max_tries)); then
			die "could not get cid, max_tries=${max_tries} reached"
		fi
		cid=$(kubectl get pods -l app="${app_label}" -o json | jq -r .items[0].status.containerStatuses[0].containerID | cut -d/ -f3)
		if [ -n "${cid}" ]; then
	...
	for i in $(seq 1 ${samples}); do
		overhead_sample=$(sudo /opt/kata/bin/kata-runtime overhead "${cid}" | grep cpu_overhead | cut -d= -f2)
